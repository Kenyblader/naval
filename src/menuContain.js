    ...
    if (isLoding)
        ...
        return (<><View style={{ flex: 1 }}>
            ...
            <View style={{ width: '100%', flexDirection: 'row', alignItems: 'center', paddingHorizontal: '45%' }}>
                <Pressable onPress={() => { navigation.navigate('Game_Online', { isHote: true, receiver: player[1]?.name, idPartie: id }) }}>
...
export const otherSalon = ({ navigation }) => {
    ...
    function connecter(id: string) {
        console.log("ok")
        rejoindrePartie(id)
            .then(() => { navigation.navigate('Game_Online', { isHote: false, receiver: id, idPartie: id }) })
